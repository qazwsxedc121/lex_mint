...
async def test_fetch_and_parse_disables_http2_when_h2_missing(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "src.infrastructure.web.webpage_service.httpx.AsyncHTTPTransport",
        lambda retries=2: object(),
    )
    monkeypatch.setattr(
...
def test_http2_supported_reflects_optional_h2_dependency(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "src.infrastructure.web.webpage_service.importlib.util.find_spec", lambda name: None
    )
    assert service._http2_supported() is False

    monkeypatch.setattr(
        "src.infrastructure.web.webpage_service.importlib.util.find_spec", lambda name: object()
    )
