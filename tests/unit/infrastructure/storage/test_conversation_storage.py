...
class TestConversationStorage:
    ...
    async def test_create_session(self, temp_conversation_dir, mock_assistant_service):
        ...
        with patch(
            ...
            return_value=mock_assistant_service,
        ):
    ...
    @pytest.mark.asyncio
    async def test_create_session_with_model_id_legacy(
        self, temp_conversation_dir, mock_assistant_service
    ):
        ...
        with patch(
            ...
            with patch(
                "src.infrastructure.config.model_config_service.ModelConfigService"
            ) as mock_model_service:
                # Mock model service
    ...
    @pytest.mark.asyncio
    async def test_create_group_session_with_group_settings(
        self, temp_conversation_dir, mock_assistant_service
    ):
    ...
    async def test_append_message_user(self, temp_conversation_dir, mock_assistant_service):
        ...
        with patch(
            ...
            await storage.append_message(
                session_id=session_id, role="user", content="What is Python?"
            )
    ...
    @pytest.mark.asyncio
    async def test_append_message_assistant_with_usage(
        self, temp_conversation_dir, mock_assistant_service
    ):
        ...
        with patch(
            ...
            cost = CostInfo(total_cost=0.0015, currency="USD")

            await storage.append_message(
                session_id=session_id, role="assistant", content="Hi there!", usage=usage, cost=cost
            )
    ...
    @pytest.mark.asyncio
    async def test_move_session_moves_compare_sidecar(
        self, temp_conversation_dir, mock_assistant_service
    ):
        ...
        with patch(
            ...
            storage = ConversationStorage(
                ...
                project_root_resolver=lambda project_id: (
                    str(project_root) if project_id == "proj-1" else None
                ),
            )
            session_id = await storage.create_session(assistant_id="default")
            ...
            moved_path = await storage._find_session_file(
                session_id, context_type="project", project_id="proj-1"
            )
    ...
    @pytest.mark.asyncio
    async def test_copy_session_copies_compare_sidecar(
        self, temp_conversation_dir, mock_assistant_service
    ):
        ...
        with patch(
            ...
            storage = ConversationStorage(
                temp_conversation_dir,
                project_root_resolver=lambda project_id: (
                    str(project_root) if project_id == "proj-2" else None
                ),
            )
            session_id = await storage.create_session(assistant_id="default")
            ...
            copied_path = await storage._find_session_file(
                copied_session_id, context_type="project", project_id="proj-2"
            )
    ...
    @pytest.mark.asyncio
    async def test_create_session_without_default_assistant_raises_clear_error(
        self, temp_conversation_dir
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_session_without_default_model_raises_clear_error(
        self, temp_conversation_dir
    ):
        with patch(
            "src.infrastructure.config.model_config_service.ModelConfigService"
            ...
            mock_service_instance.require_enabled_model = AsyncMock(
                side_effect=ValueError(
                    "No default model configured. Add a provider and model first."
                )
            )
            mock_model_service.return_value = mock_service_instance
    ...
    async def test_update_session_model(self, temp_conversation_dir):
        ...
        with patch(
            ...
            with patch(
                ...
                model_service_instance.require_enabled_model = AsyncMock(
                    return_value=(model_obj, Mock())
                )
    ...
    @pytest.mark.asyncio
    async def test_update_session_assistant_rejects_disabled_assistant(
        self, temp_conversation_dir, mock_assistant_service
    ):
        with patch(
            "src.infrastructure.config.assistant_config_service.AssistantConfigService",
            ...
            mock_assistant_service.require_enabled_assistant.side_effect = (
                _require_enabled_assistant
            )
    ...
    @pytest.mark.asyncio
    async def test_update_session_model_rejects_unavailable_model(
        self, temp_conversation_dir, mock_assistant_service
    ):
        with patch(
            "src.infrastructure.config.assistant_config_service.AssistantConfigService",
            ...
            with patch(
                ...
                async def _require_enabled_model(model_id=None):
                    ...
                    if call_count["value"] > 1 and model_id == "openai:gpt-4":
                        raise ValueError(
                            "Provider 'openai' is disabled, so model 'openai:gpt-4' is unavailable"
                        )
                    return model_obj, Mock()

                model_service_instance.require_enabled_model = AsyncMock(
                    side_effect=_require_enabled_model
                )
                mock_model_service.return_value = model_service_instance
    ...
    @pytest.mark.asyncio
    async def test_get_session_migrates_legacy_assistant_target_metadata(
        self, temp_conversation_dir, mock_assistant_service
    ):
        with patch(
            "src.infrastructure.config.assistant_config_service.AssistantConfigService",
...
""",
    ...
    @pytest.mark.asyncio
    async def test_get_session_falls_back_to_model_when_assistant_missing(
        self, temp_conversation_dir
    ):
