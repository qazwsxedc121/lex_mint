...
def test_factory_builders_only_pass_non_none_optional_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(factory, "ChatApplicationService", lambda deps: captured.setdefault("chat", deps) or ("chat", deps))
...
def test_bootstrap_helpers_and_builder(monkeypatch):
    ...
    monkeypatch.setattr(bootstrap, "settings", SimpleNamespace(attachments_dir="/tmp/attachments", max_file_size_mb=8))
    ...
    monkeypatch.setattr(bootstrap, "ComparisonStorage", lambda storage: ("comparison_storage", storage))
    monkeypatch.setattr(bootstrap, "ChatInputService", lambda storage, file_service: ("chat_input", storage, file_service))
    monkeypatch.setattr(
        bootstrap,
    ...
    monkeypatch.setattr(bootstrap, "ContextAssemblyService", lambda **kwargs: SimpleNamespace(prepare_context=("prepare_context", kwargs)))
    ...
    monkeypatch.setattr(bootstrap, "GroupOrchestrationSupportService", _GroupOrchestrationSupportService)
    ...
    monkeypatch.setattr(bootstrap, "CommitteePolicy", SimpleNamespace(resolve_committee_round_policy="round_policy"))
    monkeypatch.setattr(bootstrap, "call_llm_stream", "call_llm_stream")
    monkeypatch.setattr(bootstrap, "call_llm", "call_llm")
    monkeypatch.setattr(bootstrap, "CompareModelsOrchestrator", lambda **kwargs: ("compare_orchestrator", kwargs))
    ...
    monkeypatch.setattr(bootstrap, "build_single_chat_flow_service", lambda **kwargs: records.setdefault("single_flow", kwargs) or "single_flow")
    monkeypatch.setattr(bootstrap, "build_compare_flow_service", lambda **kwargs: records.setdefault("compare_flow", kwargs) or "compare_flow")
    monkeypatch.setattr(bootstrap, "ChatSessionCommandDeps", lambda **kwargs: kwargs)
    monkeypatch.setattr(bootstrap, "ChatSessionCommandService", lambda deps: ("session_command_service", deps))
    monkeypatch.setattr(
        bootstrap,
        "build_chat_application_service",
        lambda **kwargs: records.setdefault("chat_application", kwargs) or {"service": kwargs},
    )
    monkeypatch.setattr(bootstrap, "FileService", lambda attachments_dir, max_size: ("file_service", attachments_dir, max_size))
    monkeypatch.setattr(bootstrap, "CompressionConfigService", lambda: "compression_config")
    monkeypatch.setattr(bootstrap, "CompressionService", lambda *args, **kwargs: "compression_service")
    monkeypatch.setattr(bootstrap, "ProjectDocumentToolService", lambda: "project_document_tool_service")
    monkeypatch.setattr(bootstrap, "ProjectKnowledgeBaseResolver", lambda: "project_kb_resolver")
    monkeypatch.setattr(bootstrap, "ProjectToolPolicyResolver", lambda: "project_tool_policy_resolver")
    ...
    assert records["group_orchestration_support"]["build_rag_context_and_sources"] == "rag_context_builder"
