"""Moonshot (Kimi) OpenAI-compatible adapter implementation."""
...
class KimiAdapter(BaseLLMAdapter):
        ...
        **kwargs,
        ...
        if is_k2_5_model:
            ...
            if normalized_n not in (None, 1):
                logger.warning("Kimi K2.5 normalized n for %s: %s -> 1", model, normalized_n)
                normalized_n = 1
    ...
    async def fetch_models(self, base_url: str, api_key: str) -> list[dict[str, str]]:
        ...
        try:
            ...
            logger.warning("Failed to fetch Kimi models: %s", exc)
            return []
...
class ChatKimiOpenAI(ChatReasoningOpenAI):
    """Kimi-specific ChatOpenAI wrapper that preserves interleaved reasoning payload."""
