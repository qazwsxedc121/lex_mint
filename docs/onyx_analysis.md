...
lex_mint 当前为 FastAPI + React(Vite) 架构，路由集中在 `src/api/main.py`，会话主存储是 Markdown + YAML frontmatter（`src/infrastructure/storage/conversation_storage.py:1`）。  
RAG 与工具调用已落地，但整体仍以「单轮主流程 + 有限工具循环」为核心（`src/application/chat/single_chat_flow_service.py:82`, `src/llm_runtime/tool_loop_runner.py:53`）。
...
| 对话存储          | PostgreSQL + 结构化消息/工具调用（含 replay）                                                                                                      | Markdown 文件 + frontmatter（`src/infrastructure/storage/conversation_storage.py:1`）                                   |
| 检索/索引         | Hybrid 检索 + Federated 检索 + ACL 过滤                                                                                                      | 本地知识库检索 + Hybrid/RRF 配置（`src/infrastructure/config/rag_config_service.py:43`）                                      |
...
| Web 搜索        | 内置工具链（web_search/open_urls）+ 深度编排                                                                                                      | DuckDuckGo/Tavily + 网页抓取（`src/infrastructure/web/search_service.py:1`, `src/infrastructure/web/webpage_service.py:1`） |
...
- `SingleChatFlowService.process_message_stream` 负责 prepare-context -> stream -> persist（`src/application/chat/single_chat_flow_service.py:82`）。
- 工具循环由 `ToolLoopRunner` 驱动，默认最大 3 轮（`src/llm_runtime/tool_loop_runner.py:53`）。
...
- `src/api/main.py:19`
- `src/infrastructure/storage/conversation_storage.py:1`
- `src/application/chat/single_chat_flow_service.py:82`
- `src/llm_runtime/tool_loop_runner.py:53`
- `src/api/routers/chat.py:29`
- `src/tools/registry.py:119`
- `src/application/chat/rag_tool_service.py:47`
- `src/infrastructure/config/rag_config_service.py:43`
- `src/infrastructure/web/search_service.py:1`
- `src/infrastructure/web/webpage_service.py:1`
