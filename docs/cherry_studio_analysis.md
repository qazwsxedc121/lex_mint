...
7. [功能引入优先级建议](#7-功能引入优先级建议-结合当前落地状态)
...
| 多轮对话 | 支持 | 支持 | 无差距 |
| 消息编辑 | 完整编辑 + 重新生成 | 支持 (原地编辑 + 重新生成) | 差异小 |
| 话题分支 (Topic Branching) | 支持 | 不支持 | 较大差距 |
| 话题管理 | 支持 (对话内话题分组) | 不支持 | 中等差距 |
| 内容搜索 | 全文搜索 | 支持 (会话全文搜索) | 差异小 |
| 多选操作 | 支持 (批量操作) | 不支持 | 小差距 |
| 消息固定 | 支持 | 不支持 | 低优先级 |
| Thinking Block 展示 | 支持 (推理过程可视化) | 支持 (ThinkingBlock) | 无差距 |
| Citation Block | 支持 (引用来源展示) | 支持 (Source/RAG 引用块) | 无差距 |
...
**小结**: Cherry Studio 在话题分支与多选批量能力上仍更强；本项目已补齐搜索、Thinking/Citation 展示，并继续保持上下文压缩和后续问题建议优势。
...
| 功能 | Cherry Studio | Agents | 差距评估 |
|------|--------------|--------|---------|
| 知识库管理 | 完整 CRUD | 支持 (Knowledge Base CRUD + 文档管理) | 差距缩小 |
| 向量数据库 | LibSQL (内嵌) | ChromaDB (本地持久化) | 差异小 |
| 嵌入提供商 | OpenAI, Ollama, Voyage AI | 支持 (API/本地嵌入配置) | 差异小 |
| 文档加载器 | PDF, DOCX, Excel, Markdown, CSV, XML, EPUB, URL, Sitemap, 图片 | 支持 (TXT/MD/PDF/DOCX/HTML) | 中等差距 |
| 文档预处理 | Doc2X, MinerU, Mistral, PaddleOCR | 基础文本抽取 + 语义切块 | 中等差距 |
| 重排序 | Jina, Bailian, TEI, Voyage | 暂不支持 | 差距 |
| 混合搜索 | 向量 + 全文 | 暂不支持 (向量检索为主) | 差距 |
| Agent 绑定知识库 | 支持 | 支持 (assistant `knowledge_base_ids`) | 无差距 |

**小结**: 本项目已落地可用的 RAG 主链路（知识库 CRUD、文档上传处理、向量检索、助手绑定），当前主要差距集中在重排序与混合检索策略。

### 4.6 翻译工具

| 功能 | Cherry Studio | Agents | 差距评估 |
|------|--------------|--------|---------|
| 独立翻译页面 | 支持 (专用 UI) | 无 (设置项 + 对话内翻译) | Cherry Studio 特色 |
| 语言自动检测 | 支持 (Franc 库) | 暂不支持 | 差距 |
| 100+ 目标语言 | 支持 | 基础支持 (可配置目标语言) | 中等差距 |
| 文档翻译 (OCR) | 支持 | 无 | 差距 |
| 翻译历史 | 支持 | 无 | 差距 |
| 双向滚动同步 | 支持 | 无 | 差距 |

**小结**: 本项目已具备对话内翻译能力和独立翻译配置，但尚未形成独立翻译工作台与历史/文档处理体系。
...
| ChatGPT 数据导入 | 支持 | 支持 (.json/.zip 导入) | 差距缩小 |
...
## 7. 功能引入优先级建议 (结合当前落地状态)

### P0 - 基础必备 (未完成项)

| 功能 | 工程量 | 当前状态 | 建议 |
|------|--------|---------|------|
| Docker 容器化 | 低 | 未开始 | 继续保留 P0，优先补齐部署标准化 |
| 对话文件夹组织 | 中 | 时间分组已完成 | 自定义文件夹为下一步, 详见 conversation_folder_plan.md |

### P1 - 高价值功能 (增强项)

| 功能 | 工程量 | 当前状态 | 建议 |
|------|--------|---------|------|
| 记忆系统 (简化版) | 中 | 未开始 | 下一阶段优先，与 LangGraph 优势契合 |
| 消息块组件系统完善 | 中 | 部分完成 (Thinking/Citation 已有) | 补齐 Tool/Memory 等块 |
| 基础 RAG 增强 | 中 | 已完成主链路 | 增补重排序、混合检索 |

### P2 - 中等价值功能 (中长期)

| 功能 | 工程量 | 当前状态 | 建议 |
|------|--------|---------|------|
| MCP 客户端支持 | 高 | 未开始 | 作为平台能力长期投入 |
| 话题分支 | 高 | 未开始 | 结构改造较大，放中后期 |
| KaTeX 数学公式 | 低 | 未开始 | 可穿插实现，性价比较高 |

### P3 - 锦上添花

| 功能 | 工程量 | 当前状态 | 建议 |
|------|--------|---------|------|
| 图像生成 | 中 | 未开始 | 结合用户画像决定是否推进 |
| 笔记系统 | 高 | 未开始 | 功能独立，建议单独立项 |
| Mini Apps | 中 | 未开始 | 依赖整体平台化路线 |
...
- [x] Thinking Block 展示 (推理过程可视化)
- [x] 对话导出功能 (Markdown / JSON 下载)
- [x] 消息原地编辑 (前端改造)
- [~] 对话文件夹组织 (时间分组已完成, 自定义文件夹待实现, 详见 conversation_folder_plan.md)

**阶段进度**: 3/5 (60%)

### 8.2 Phase 2 - 中期 (核心增值)

- [x] 基础 RAG 能力 (文档上传 + 嵌入 + 检索)
- [~] 消息块组件系统 (Thinking / Citation 已实现，Tool 待补)
...
- Projects 工作区支持“关闭当前项目”并返回 Welcome 状态。
- Welcome 页面支持直接 `Add Project`，可一步进入创建表单。
- `Select Server Directory` 支持在服务端目录浏览器中新建文件夹。
- 选择目录后若 Project Name 为空，自动回填为目录名，降低从零创建项目门槛。

### 8.5 总体进度判断

- 已从“分析阶段”进入“持续交付阶段”，核心聊天与配置体系稳定。
- RAG、翻译、TTS、项目工作区等能力已形成可用闭环。
- 下一阶段应聚焦平台化缺口：Docker、记忆系统、MCP、话题分支。
...
| 文件 | 描述 |
|------|------|
| `src/api/main.py` | 后端入口，17 个路由 (含 RAG/翻译/TTS/Prompt Templates) |
...
| RAG | LibSQL 向量 | PostgreSQL 嵌入 | 13 种向量 DB (最强) | ChromaDB + 知识库管理 |
| Agent 编排 | 基础 (会话式) | Agent Runtime | 简单循环 | LangGraph 状态机 (最强) |
| 翻译 | 独立模块 (最强) | 消息翻译 | 无 | 对话内翻译 + 翻译配置 |
| 图像生成 | 独立模块 | 支持 | 支持 | 无 |
...
| 代码复杂度 | 高 | 极高 | 高 | 低 (最简洁) |
| 独有特色 | 选择助手/翻译/Mini Apps | 插件市场/Agent 市场 | 13 种向量 DB | 上下文压缩/后续建议/费用追踪/项目工作区 |

### 9.4 分析版本信息

- **Cherry Studio**: v1.7.17，源码位于 `learn_proj/cherry-studio`
- **Agents 项目**: master 分支，最新提交 `f36574e`
- **本次状态同步**: 已纳入 Projects 工作区近两次迭代（关闭项目回 welcome、Welcome 直达 Add Project、服务端目录选择器可新建目录、目录回填自动带出 Project Name）
