...
class UpdateSessionFolderRequest(BaseModel):
    ...
    folder_id: str | None = None  # None to remove from folder
            ...
            if session.get("folder_id") == folder_id:
                try:
                    await storage.update_session_folder(
                        session_id=session["session_id"], folder_id=None, context_type="chat"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to clear folder_id for session {session['session_id']}: {e}"
                    )
