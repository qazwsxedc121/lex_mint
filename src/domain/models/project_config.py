...
class ProjectRagSettings(BaseModel):
    ...
    knowledge_base_ids: list[str] = Field(
        default_factory=list, description="Knowledge base IDs available to this project"
    )
...
class Project(BaseModel):
    ...
    settings: ProjectSettings = Field(
        default_factory=ProjectSettings, description="Project-specific settings"
    )
...
class ProjectWorkspaceItemUpsert(BaseModel):
    ...
    @field_validator("path")
    @classmethod
    def validate_optional_item_path(cls, value: str | None) -> str | None:
        if value is None:
            return None
        return _normalize_project_relative_path(value)
